...
VALID_RESULTS = {"PASS", "PARTIAL", "UNTESTED", "FAIL"}
...
def run_case(case: dict, command_template: str, timeout: int) -> dict:
    ...
    for criterion in criteria:
        ...
        criterion_results.append({
            ...
            "status": "OBSERVED" if observed and not not_observed else "NOT_OBSERVED",
        })

    observed_count = sum(item["observed"] for item in criterion_results)
    if timed_out:
        result = "FAIL"
    elif observed_count == len(criteria) and exit_code == 0:
        result = "PASS"
...
def main() -> int:
    ...
    results = [run_case(case, args.command, args.timeout) for case in cases]
    document = {
