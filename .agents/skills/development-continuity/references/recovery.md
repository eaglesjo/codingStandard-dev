...
Branch and tag names are coordination names and can move. Before applying a payload, publishing a commit, or performing a destructive operation, compare the expected immutable identity with the actual base. If the base moved, recover the new state and deliberately rebase, merge, or recreate the payload.
