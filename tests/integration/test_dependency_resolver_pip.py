...
The fixture is self-contained: wheels are generated in a temporary directory,
so the test never needs a package index or modifies the test environment.
"""
...
def _wheel_bytes(name: str, version: str, requires: list[str] | None = None) -> bytes:
    ...
    files = {
        ...
        f"{dist_info}/METADATA": metadata_text.encode(),
        f"{dist_info}/WHEEL": wheel_text.encode(),
    }
    ...
    for path, payload in files.items():
        ...
        record_rows.append((path, f"sha256={encoded}", str(len(payload))))
...
def _write_wheel(directory: Path, name: str, version: str, requires: list[str] | None = None) -> None:
    ...
    path.write_bytes(_wheel_bytes(name, version, requires))
...
class PipDependencyResolverTests(unittest.TestCase):
    def test_selected_library_drives_affected_dependency_resolution(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            wheelhouse = Path(tmp)
            _write_wheel(wheelhouse, "compat-lib", "1.0.0")
            _write_wheel(wheelhouse, "compat-lib", "2.0.0")
            _write_wheel(wheelhouse, "anchor-lib", "2.0.0", ["compat-lib>=2.0.0"])
            ...
            self.assertRegex(combined, r"anchor-lib-2\.0\.0")
            self.assertRegex(combined, r"compat-lib-2\.0\.0")
            self.assertNotRegex(combined, r"compat-lib-1\.0\.0")
    ...
    def test_real_pip_resolver_rejects_incompatible_pinned_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertRegex(completed.stderr + completed.stdout, r"(?i)(conflict|incompatible|resolutionimpossible|cannot install)")
