...
def check_notebook() -> None:
    for path in (ROOT / "tests" / "colab" / "codingstandard_colab_test.ipynb", ROOT / "examples" / "colab" / "clean_runtime_validation.ipynb", ROOT / "examples" / "colab" / "llm_qlora_validation.ipynb"):
        ...
        if notebook.get("nbformat") != 4:
            fail(f"Colab notebook must use nbformat 4: {path}")
        if not notebook.get("cells"):
            fail(f"Colab notebook has no cells: {path}")
...
def check_routing_paths() -> None:
    for rel in ROUTING_FILES:
        ...
        for legacy in LEGACY_EXECUTION_REFS:
            if legacy in text:
                fail(f"Obsolete execution reference '{legacy}' remains in {rel}")
...
def check_version_consistency() -> None:
    ...
    if not match:
        fail("core/common/environment.py is missing STANDARD_VERSION")
    if match.group(1) != version:
        fail(f"Version mismatch: VERSION={version}, core/common/environment.py={match.group(1)}")
