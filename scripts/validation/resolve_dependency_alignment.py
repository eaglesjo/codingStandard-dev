...
def resolve_alignment(scenario: dict) -> dict:
    ...
    for package, constraint in constraints:
        ...
        compatible = [
            version
            for version in candidates.get(package, [])
        ...
        if satisfies(current, constraint):
            ...
            # Deterministic contract: preserve a compatible current version;
            # otherwise choose the lowest compatible candidate to avoid
            # introducing unnecessary version drift.
            aligned[package] = min(compatible, key=version_key)
